...
def _task_is_solved(task: Any, idx: int, y: str) -> bool:
    try:
...
def _parse_proposals(task: Any, parent_y: str, response: str, step: int) -> list[str]:
    if not response:
        return []
    try:
...
def _dedup_proposals(task: Any, proposals: list[str]) -> list[str]:
...
def _get_value(task: Any, x: str, y: str, n_evaluate_sample: int, model_name: str, task_id: int, step: int, parent_id: Any, branch_id: Any) -> float:
...
def _run_step0(task: Any, x: str, args, task_id: int) -> tuple[list[str], float, dict[str, Any]]:
    start_t = time.time()
...
def _run_serial(task: Any, idx: int, args, log_dir: str) -> dict[str, Any]:
    task_id = idx + 1
    ...
    result = {
        ...
        "baseline": {
            ...
            "usage_by_call_type": dict(usage.get("by_call_type") or {}),
            "total_time": float(total_time),
            "search_time": float(search_time),
            "best_solution": {"y": best_y, "node_id": None, "depth": None, "value": None},
        },
    }
...
def _run_scheme_h(task: Any, idx: int, args, log_dir: str) -> dict[str, Any]:
    task_id = idx + 1
    x = task.get_input(idx)
...
def _run_dfs(task: Any, idx: int, args, log_dir: str) -> dict[str, Any]:
    task_id = idx + 1
    x = task.get_input(idx)
    ...
    def _dfs(parent_y: str, depth: int) -> tuple[bool, str]:
        ...
        for i in order:
            ...
            if best_solution.get("value") is None or v > float(best_solution.get("value") or 0.0):
                best_solution.update({"y": child_y, "value": v, "depth": child_step})
...
def _run_layered(task: Any, idx: int, args, log_dir: str) -> dict[str, Any]:
    task_id = idx + 1
    x = task.get_input(idx)
...
def _run_scheme_b(task: Any, idx: int, args, log_dir: str) -> dict[str, Any]:
    task_id = idx + 1
    x = task.get_input(idx)
