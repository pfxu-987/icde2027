...
class BirdPlanProxyTask(BirdDecompRepairTask):
    """BIRD decomp-repair variant with a SQLite EXPLAIN QUERY PLAN cost proxy.

    This branch leaves the existing BirdDecompRepairTask untouched. It adds a
    side-effect-free physical probe before LLM value scoring:
    - EXPLAIN QUERY PLAN failures are hard-pruned.
    - Surviving candidates get C_db from a deterministic plan proxy.
    """
    ...
    def calibrate_proxy(self, start_idx: int = 0, end_idx: int | None = None) -> dict[str, Any]:
        end_idx = len(self.data) if end_idx is None else min(int(end_idx), len(self.data))
        costs_by_db: dict[str, list[float]] = {}
        for idx in range(max(0, int(start_idx)), end_idx):
            db_id = self._db_id(idx)
            sql = self._gold_sql(idx)
            ok, cost, _meta = self._proxy_cost(db_id, sql)
            if ok:
                costs_by_db.setdefault(db_id, []).append(float(cost))

        all_costs: list[float] = []
        with self._proxy_lock:
            for db_id, values in costs_by_db.items():
                self._proxy_bmax_by_db[db_id] = _p95(values)
    ...
    def calibrate_proxy_from_schema(self, db_ids: list[str] | None = None) -> dict[str, Any]:
        """Build per-database B_proxy budgets without using gold SQL.

        SQLite has no native optimizer cost in EXPLAIN QUERY PLAN, so this
        branch estimates a physical budget from each database's table sizes.
        The generated samples mimic common plan shapes in this task:
        single-table scans, scan+sort/group temp structures, and small joins.
        """
        if db_ids is None:
    ...
    def _schema_proxy_budget_samples(self, table_counts: list[int]) -> list[float]:
        ...
        for rows in counts:
            ...
            costs.append(base + 1.0)  # one full table scan
            costs.append(base + 2.0)  # scan plus temp B-tree for sort/group

        top = counts[: min(8, len(counts))]
        for i, left in enumerate(top):
            for right in top[i + 1 :]:
                base = math.log1p(left + right)
                costs.append(base + 3.0)  # two accesses plus one join
                costs.append(base + 4.0)  # join plus temp B-tree

        for i in range(max(0, len(top) - 2)):
            rows = sum(top[i : i + 3])
            costs.append(math.log1p(rows) + 5.0)  # three accesses, two joins
    ...
    def _proxy_cost(self, db_id: str, sql: str) -> tuple[bool, float, dict[str, Any]]:
        ...
        try:
            ...
            return False, 0.0, {"reason": "explain_failure", "error": str(e)}
        ...
        for row in rows:
            ...
            if upper.startswith("SCAN "):
                n_scan += 1
                n_access += 1
                m = re.search(r"\bSCAN\s+(?:TABLE\s+)?[`\"[]?([A-Za-z_][A-Za-z0-9_]*)", detail, flags=re.IGNORECASE)
                if m:
                    full_scan_tables.add(m.group(1))
